<?php
	...
	{
		mysql_query("UPDATE alternatif SET nama_alternatif = '$_POST[nama_alternatif]', ket = '$_POST[ket]'  WHERE kode_alternatif = '$_POST[kode_alternatif]'");

		for($i=0;$i < count($_POST['kriteria']); $i++){
			$kode_kriteria = $_POST[kode_kriteria][$i];
			$nilai  			 = $_POST[kriteria][$i];

			$query = mysql_query("SELECT * FROM nilai WHERE kode_alternatif = '$_POST[kode_alternatif]' AND kode_kriteria = '$kode_kriteria'");
			if (!$data = mysql_fetch_array($query))
			{
				mysql_query("INSERT INTO nilai(id_nilai, kode_alternatif, kode_kriteria, nilai) VALUES('0','$_POST[kode_alternatif]', '$kode_kriteria', '$nilai')");
			}
			mysql_query("UPDATE nilai SET nilai = '$nilai' WHERE kode_alternatif = '$_POST[kode_alternatif]' AND kode_kriteria = '$kode_kriteria'");
						...
						<table width="1000" class="table">
						  <tr>
						    <td align="center" valign="top" bgcolor="#FFFFFF"><br />
						      ...
						      <br />
						      <?php
									$queryalternatif = mysql_query("SELECT * FROM alternatif WHERE kode_alternatif = '$_GET[kode_alternatif]'");
						          ...
						          <tr>
						            ...
						            <td width="267" bgcolor="#FFFFFF"><input type="text" class="form-control" name="ket" id="ket" value="<?php echo $dataalternatif['ket']; ?>" /></td>
											...
											while ($dt_k = mysql_fetch_assoc($sql_k)) {
												$sql_nilai = mysql_query("Select * from nilai where kode_alternatif = '$_GET[kode_alternatif]' and kode_kriteria = '$dt_k[kode_kriteria]'");
												$dt_nilai = mysql_fetch_assoc($sql_nilai);
													if ($dt_nilai['nilai'] == 0) {
															$nilai = 0;
													}else{
